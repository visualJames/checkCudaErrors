/*
*this string is not needed yet for the checkErrorFunction,
*but maybe it can improve it :)
*It was created with regex PCRE (PHP)
*Here is where I learnt it: https://regex101.com/ http://2017.compciv.org/guide/topics/end-user-software/atom/how-to-use-regex-atom.html
*I have used for this ^CUDA_[a-zA-Z_]+\s=\s[0-9]{1,3},   ^(\w|.)+,
* ^(,CUDA[A-Z_]+\s=\s[0-9]{1,3}\n)(,), ^",(CUDA[A-Z_]+\s=\s[0-9]{1,3})"\n,([a-zA-Z\s.0-9.(),-_']+)
* source for the errors are https://docs.nvidia.com/cuda/cuda-driver-api/group__CUDA__TYPES.html#group__CUDA__TYPES
*/

std::string error[] = {"CUDA_SUCCESS = 0"
